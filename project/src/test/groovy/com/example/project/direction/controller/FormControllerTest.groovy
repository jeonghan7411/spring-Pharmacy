...
class FormControllerTest extends Specification {
    ...
    def "GET /"() {
        //  간단한 테스트인경우 given,when,then대신  expect하나로 가능함
        expect:
    ...
    def "POST /search"() {
        ...
        1 * pharmacyRecommendationService.recommendPharmacyList(argument -> {
            assert argument == inputAddress // mock 객체의 argument 검증  (입력받은 값이 (controller의 파라미터 값) 서비스에 전달하는 값이랑 동일한지 검증)
        }) >> outputDtoList
        // >>  뜻은 앞의 메소드가호출이 되면  outputDtoList로 리턴을 하겠다는 뜻
